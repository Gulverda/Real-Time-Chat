...
const Chat = () => {
    ...
    // Use a separate function to check for unread messages in the friend list
    const handleFriendClick = (friendUsername) => {
        fetchPrivateMessages(friendUsername);
        // If the recipient is already selected, do nothing
        if (recipient !== friendUsername) {
            setMessages([]); // Optionally, clear previous chat messages
    ...
    return (
        <div className="p-4 gap-5 md:flex grid max-w-[1440px] w-full justify-center md:mt-20">
            ...
            <div className="max-w-[800px] w-full p-4 bg-[#FDFDFF] shadow-sm border border-[#E1E2FF] rounded-lg md:flex grid h-[500px]">
                ...
                {/* Chat Section */}
                <div className="md:w-2/3 w-full flex flex-col p-4 overflow-y-auto">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                        ...
                        />
                        <button onClick={sendPrivateMessage} className="bg-[#5D5FEF] text-white px-4 py-2 rounded-lg hover:bg-[#7275cb] w-[50px] max-w-[50px] h-[42px]">
